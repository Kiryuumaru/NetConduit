...
public sealed class StreamMultiplexer : IStreamMultiplexer
{
    ...
    internal async Task RunAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Start retransmit task only if timeout-based retransmit is enabled
            if (_options.EnableTimeoutRetransmit)
                _retransmitTask = RetransmitLoopAsync(ct);

            // Wait for handshake to complete
            await _handshakeCompleted.Task.WaitAsync(ct).ConfigureAwait(false);

            // Wait for all tasks to complete
            var taskList = new List<Task> { _readTask, _writeTask, _pingTask };
            if (_flushTask != null) taskList.Add(_flushTask);
            if (_retransmitTask != null) taskList.Add(_retransmitTask);
    ...
    private async Task RetransmitLoopAsync(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            try
            {
                await Task.Delay(_options.RetransmitCheckInterval, ct).ConfigureAwait(false);
                
                // Check all write channels for frames needing retransmission
                foreach (var kvp in _writeChannelsByIndex)
                {
                    var channel = kvp.Value;
                    ...
                    try
                    {
                        var shouldClose = await channel.CheckAndRetransmitAsync(ct).ConfigureAwait(false);
                        
                        if (shouldClose)
                        {
                            // Max retransmit attempts exceeded - close channel with error
                            channel.SetError(ErrorCode.Timeout, $"Max retransmit attempts ({_options.MaxRetransmitAttempts}) exceeded");
                        }
                    }
                    catch (OperationCanceledException)
                    {
                    ...
                    {
                        // Ignore errors on individual channels
                    }
                }
            }
            catch (OperationCanceledException) when (ct.IsCancellationRequested)
            ...
            {
                // Ignore errors in retransmit loop
            }
        }
    }
