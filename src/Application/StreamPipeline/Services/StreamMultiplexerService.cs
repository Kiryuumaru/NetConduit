...
[Disposable]
public partial class StreamMultiplexerService(ILogger<StreamMultiplexerService> logger)
{
    private readonly ILogger<StreamMultiplexerService> _logger = logger;
            ...
            Task.Run(async () =>
            {
                while (!ct.IsCancellationRequested && !_mainTranceiverStream.IsDisposedOrDisposing)
                {
                    try
                    {
                        ...
                        await _tranceiverStreamMap[channel].ReceiverStream.WriteAsync(receivedBytes[_headerSize..], ct);
                    }
                    catch (Exception ex)
                    { 
    ...
    private async void StartPipe(Guid channelKey, TranceiverStream tranceiverStream, CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                await tranceiverStream.SenderStream.ReadAsync(receivedBytes.Slice(_headerSize, _bufferSize.Value), stoppingToken);
