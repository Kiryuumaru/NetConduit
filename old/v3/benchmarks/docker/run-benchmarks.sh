...
go build -o "$RESULTS_DIR/go-bench" .
...
taskset "$CPU_MASK" dotnet run \
    ...
    > "$RESULTS_DIR/dotnet-gametick.json" || echo "  WARNING: .NET game-tick exited with error"
...
taskset "$CPU_MASK" dotnet run \
    ...
    > "$RESULTS_DIR/dotnet-throughput.json" || echo "  WARNING: .NET throughput exited with error"
