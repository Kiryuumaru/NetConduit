        ...
        try
        {
            ...
            mux.OnAutoReconnecting += (args) =>
            {
                if (args.IsReconnecting)
                    Console.WriteLine($"[Server] Awaiting reconnection (attempt {args.AttemptNumber}/{args.MaxAttempts})...");
            };
            mux.OnReconnected += () =>
                Console.WriteLine("[Server] Player reconnected — session restored");
            mux.OnAutoReconnectFailed += (ex) =>
                Console.WriteLine("[Server] Reconnection window expired — session ended");
    ...
    CancellationToken ct)
{
    ...
    var leaderboardWriteChannel = await mux.OpenChannelAsync(new ChannelOptions
    {
...
async Task RunPlayerAsync(string playerHost, int playerPort, string name, CancellationToken ct)
{
    ...
    {
        if (args.IsReconnecting)
            Console.WriteLine($"[{name}] Reconnecting (attempt {args.AttemptNumber})...");
    };
    mux.OnReconnected += () =>
