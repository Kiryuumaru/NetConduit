...
/// <summary>
/// Comprehensive scalability benchmark testing channel count × data size matrix.
/// Compares raw TCP connections vs multiplexed channels across different scenarios.
/// </summary>
...
public class ScalabilityBenchmark
{
    ...
    /// <summary>
    /// Number of concurrent channels/connections.
    /// Tests from single channel to high concurrency.
    /// </summary>
    ...
    {
        _sendBuffer = new byte[DataSizeBytes];
        Random.Shared.NextBytes(_sendBuffer);
    }

    /// <summary>
    /// Baseline: N separate TCP connections, each transferring DataSizeBytes
    /// </summary>
    ...
    public async Task RawTcp()
    {
        ...
        try
        {
                ...
                for (int i = 0; i < ChannelCount; i++)
                {
                    ...
                    tasks.Add(Task.Run(async () =>
                    {
                        try
                        {
                            ...
                            var recvBuffer = new byte[DataSizeBytes];
                            int totalRead = 0;
                            while (totalRead < DataSizeBytes)
                            {
                                var read = await stream.ReadAsync(recvBuffer.AsMemory(totalRead), cts.Token);
                                if (read == 0) break;
                                totalRead += read;
                            }
                        }
            ...
            for (int i = 0; i < ChannelCount; i++)
            {
                clientTasks.Add(Task.Run(async () =>
                {
                    ...
                    using var stream = client.GetStream();
                    await stream.WriteAsync(_sendBuffer, cts.Token);
                }, cts.Token));
            }
        ...
        {
            listener.Stop();
            await Task.Delay(50); // Allow port release
        }
    }

    /// <summary>
    /// Multiplexer: 1 TCP connection with N channels, each transferring DataSizeBytes
    /// </summary>
        ...
        try
        {
            var serverTask = Task.Run(async () =>
            {
                ...
                await foreach (var channel in server.Multiplexer.AcceptChannelsAsync(cts.Token))
                {
                    ...
                    readTasks.Add(Task.Run(async () =>
                    {
                        ...
                        while (totalRead < DataSizeBytes)
                        {
                            var read = await ch.ReadAsync(recvBuffer.AsMemory(totalRead), cts.Token);
                            if (read == 0) break;
                            totalRead += read;
                        }
                    }, cts.Token));
            ...
            var clientTask = Task.Run(async () =>
            {
                ...
                for (int i = 0; i < ChannelCount; i++)
                {
                    ...
                    {
                        await ch.WriteAsync(_sendBuffer, cts.Token);
