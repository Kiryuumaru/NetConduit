    ...
    foreach (var channels in channelCounts)
    {
        foreach (var dataSize in dataSizes)
        {
            try
            {
                ...
                results.Add(MakeResult("Raw TCP (.NET)", "throughput", channels, dataSize, rawMbps, 0));
            ...
            try
            {
                ...
                results.Add(MakeResult("NetConduit Mux TCP", "throughput", channels, dataSize, muxMbps, 0));
    ...
    foreach (var channels in gtChannels)
    {
        foreach (var msgSize in msgSizes)
        {
            try
            {
                ...
                results.Add(MakeResult("Raw TCP (.NET)", "game-tick", channels, msgSize, 0, rawMps));
            ...
            try
            {
                ...
                results.Add(MakeResult("NetConduit Mux TCP", "game-tick", channels, msgSize, 0, muxMps));
...
static object MakeResult(string impl, string scenario, int channels, int dataSize, double mbps, double mps)
{
    return new
    {
        implementation = impl,
        ...
        totalBytes = scenario == "throughput" ? (long)channels * dataSize : 0L,
        durationMs = mbps > 0 ? (long)channels * dataSize / mbps / 1_048_576.0 * 1000.0 : 0.0,
        throughputMBps = mbps,
        messagesPerSec = mps,
        allocMB = 0.0
    };
    ...
    for (int run = 0; run < runs; run++)
    {
        ...
        var server = StreamMultiplexer.Create(new MultiplexerOptions
        {
            StreamFactory = async ct =>
            {
                ...
                tcp.NoDelay = true;
                return new StreamPair(tcp.GetStream());
            }
        });
